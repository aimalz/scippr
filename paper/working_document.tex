...
To avoid making an analysis pipeline that depends on the choice of supernova classification lightcurve fitting, and photometric redshift assignment techniques, we create mock data in the form of a catalog of interim posteriors over the latent variables.
...
Here we outline the construction of interim posteriors for each supernova.  There are many ways to formulate the various terms in Eqs. \ref{eq:first} and \ref{eq:second}, but we will restrict ourselves to the simplest version for now, outlined below in Sec. \ref{sec:choices}.  

\subsubsection{Setting true values of the hyperparameters}
\label{sec:true_hypers}

We begin by setting true values of the hyperparameters.  $\vec{\theta}$ represents the generating cosmology of our simulation, a vector comprised of $\Omega_{\Lambda}$, $\Omega_{m}$, $H_{0}$, etc.  $\textul{\phi}$ represents the parameters defining the supernova type proportions as a function of redshift, which we choose to be a $T\times Z$ dimensional matrix for a space with $T$ possible supernova types and $Z$ redshift bins, where each element of $\phi_{\tau\zeta}$ is the probability density associated with a supernova of type $\tau$ with a redshift in bin $\zeta$; thus we have $\phi_{\tau\zeta}=p(z, t | $.  It is at this stage that we set true values of $\vec{\theta}_{0}$ and $\textul{\phi}_{0}$ that we would like to recover by way of hierarchical inference on mock data derived from them.  

We will first draw pairs of true parameters $(T_{n}^{0}, z_{n}^{0})$ from $\textul{\phi}_{0}$.  Then we will calculate the true $\mu_{n}^{0}$ according to [insert distance modulus equation here ({\bf astropy.cosmology})] from $z_{n}^{0}$ and $\vec{\theta}_{0}$.
...
\subsection{Relative Supernova Rates}
